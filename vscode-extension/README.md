...
3. Run **1C: Настроить отладку…** from the Command Palette. The click-through wizard adds a separate configuration to the selected workspace folder’s `.vscode/launch.json`; existing launch configurations are preserved.
   ...
   4. **Debug mode and review.** Choose launch or attach. For a file-base launch, choose either the normal 1C client (`launchMode: "client"`, with a temporary `dbgs`) or a local standalone server (`launchMode: "standaloneServer"`, `ibsrv` with HTTP debugging). In standalone mode the adapter starts the thin client (`1cv8c`) through `/WS http://127.0.0.1:<port>`, not a web client. A numeric loopback avoids macOS IPv6 `localhost` fallback issues. The standalone server keeps its isolated state under `.vscode/onec-standalone-server`, uses a dedicated direct port, and stops with the debug session. SSH stays disabled because it is unnecessary for debugging and requires a host key on macOS/Linux. Then set the debug server address and confirm the generated configuration.
