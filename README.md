...
3. В палитре команд VS Code запустите **«1C: Настроить отладку…»**. Это интерактивный мастер: он показывает только пригодные для запуска платформы, зарегистрированные базы и найденные исходники, а затем добавляет отдельную конфигурацию в `.vscode/launch.json`, не удаляя существующие.
   ...
   2. **Информационная база.** Мастер читает стандартный список запуска 1С `ibases.v8i` и даёт выбрать базу мышью. В новой или пустой папке можно нажать **«Выбрать файл ibases.v8i…»** и указать список запуска из любого расположения: его базы появятся в том же списке для выбора. В Windows проверяются профили `%APPDATA%` и `%LOCALAPPDATA%`, в macOS — `~/Library/Application Support/1C/1CEStart` и `~/.1cv8/1C/1CEStart`, в Linux — `~/.1cv8/1C/1CEStart`. Также можно выбрать незарегистрированный каталог файловой базы или ввести имя зарегистрированной базы.
