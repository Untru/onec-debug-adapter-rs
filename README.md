...
3. В палитре команд VS Code запустите **«1C: Настроить отладку…»**. Это интерактивный мастер: он показывает только пригодные для запуска платформы, зарегистрированные базы и найденные исходники, а затем добавляет отдельную конфигурацию в `.vscode/launch.json`, не удаляя существующие.
   ...
   4. **Режим и подтверждение.** Выберите запуск либо подключение. Для запуска файловой базы мастер отдельно предлагает обычный клиент 1С (`launchMode: "client"`, временный `dbgs`) или автономный сервер (`launchMode: "standaloneServer"`, локальный `ibsrv` с HTTP-отладкой). Второй вариант создаёт собственные данные сервера в `.vscode/onec-standalone-server`, использует выделенный порт прямого подключения и останавливается вместе с отладочной сессией. SSH не включается: для отладки он не нужен и на macOS/Linux требует отдельного ключа хоста. Затем выберите адрес сервера отладки и подтвердите просмотренную конфигурацию.
