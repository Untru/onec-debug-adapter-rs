...
3. В палитре команд VS Code запустите **«1C: Настроить отладку…»**. Это интерактивный мастер: он показывает только пригодные для запуска платформы, зарегистрированные базы и найденные исходники, а затем добавляет отдельную конфигурацию в `.vscode/launch.json`, не удаляя существующие.
   ...
   2. **Информационная база.** Мастер читает стандартный список запуска 1С `ibases.v8i` и даёт выбрать базу мышью. В новой или пустой папке можно нажать **«Выбрать файл ibases.v8i…»** и указать список запуска из любого расположения: его базы появятся в том же списке для выбора. Основной каталог запуска — `%APPDATA%/1C/1CEStart` в Windows и `~/.1C/1cestart` в macOS/Linux; мастер также читает перечисленные в соседнем `1cestart.cfg` (UTF-8 или UTF-16) общие списки `CommonInfoBases`. Старые расположения остаются fallback. Имя записи используется только при выборе: мастер записывает в конфигурацию абсолютный путь файловой базы либо безопасное серверное подключение `Srvr="сервер";Ref="база";`, без логина и пароля. Также можно выбрать незарегистрированный каталог файловой базы или ввести такое серверное подключение вручную.
...
Для серверной базы укажите в `infoBase` строку `Srvr="сервер";Ref="база";`. Адаптер запустит тонкий клиент с `/Sсервер\база` и использует `Ref` как RDBG-псевдоним; `infoBaseAlias` нужен только чтобы явно переопределить этот псевдоним.
