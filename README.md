...
3. В палитре команд VS Code запустите **«1C: Настроить отладку…»**. Это интерактивный мастер: он показывает только пригодные для запуска платформы, зарегистрированные базы и найденные исходники, а затем добавляет отдельную конфигурацию в `.vscode/launch.json`, не удаляя существующие.
   ...
   В multi-root workspace сначала выбирается папка, в которой будет создан `launch.json`. Поиск исходников идёт только внутри неё; внешний каталог можно добавить через **«Выбрать каталог…»**. Мастер не записывает пароль, строку подключения с учётными данными или трассировку. Для базы из `ibases.v8i` в `launch.json` сохраняется лишь её имя; сохранённые настройками запуска 1С учётные данные остаются в `ibases.v8i` и не отображаются.
